...
"""
Simplified integration tests for usage policy configuration in Anthropic Messages API.
...
class TestUsagePolicyDefault:
    ...
    async def test_streaming(self, client: anthropic.AsyncAnthropic):
        ...
        # message_start should have usage with input_tokens set
        assert message_start_usage is not None, "message_start should have usage"
        assert message_start_usage.input_tokens > 0, (
            "message_start usage should have input_tokens > 0"
        )
        assert message_start_usage.output_tokens == 0, (
            "message_start usage should have output_tokens == 0"
        )

        # message_delta should have complete usage
        assert message_delta_usage is not None, "message_delta should have usage"
        assert message_delta_usage.input_tokens > 0, (
            "message_delta usage should have input_tokens > 0"
        )
        assert message_delta_usage.output_tokens > 0, (
            "message_delta usage should have output_tokens > 0"
        )

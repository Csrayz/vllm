...
def default_server():
    ...
    try:
        with RemoteOpenAIServer(
            MODEL_NAME, DEFAULT_SERVER_ARGS, auto_port=False
        ) as srv:
            server = srv
...
@pytest.mark.asyncio
async def test_default_non_streaming(default_client: openai.AsyncOpenAI, winning_call):
    """Default behavior: Non-streaming should always return usage in response."""
    logger.info("TEST: Default non-streaming")

    response = await default_client.audio.transcriptions.create(
        model=MODEL_NAME,
        file=winning_call,
        language="en",
        temperature=0.0,
    ...
    logger.info("Response usage: %s", out.get("usage"))
    assert "usage" in out, "Default non-streaming should have usage in response"
    assert out["usage"]["prompt_tokens"] > 0
    logger.info("TEST: Default non-streaming PASSED")


@pytest.mark.asyncio
async def test_default_streaming_no_usage(
    default_client: openai.AsyncOpenAI, winning_call
):
    ...
    stream = await default_client.audio.transcriptions.create(
        model=MODEL_NAME,
        file=winning_call,
        language="en",
        temperature=0.0,
    ...
    logger.info(
        "Total chunks: %s, Usage in chunks: %s",
        chunk_count,
        usage_in_chunks,
    )
...
def always_server():
    ...
    server = None
    try:
        with RemoteOpenAIServer(MODEL_NAME, ALWAYS_SERVER_ARGS, auto_port=False) as srv:
            server = srv
...
@pytest.mark.asyncio
async def test_always_non_streaming(always_client: openai.AsyncOpenAI, winning_call):
    """Always policy: Non-streaming should return usage."""
    logger.info("TEST: Always policy non-streaming")

    response = await always_client.audio.transcriptions.create(
        model=MODEL_NAME,
        file=winning_call,
        language="en",
        temperature=0.0,
    ...
    out = json.loads(response)
    logger.info("Response usage: %s", out.get("usage"))
    assert "usage" in out, "Always policy non-streaming should have usage"
    assert out["usage"]["prompt_tokens"] > 0
    logger.info("TEST: Always policy non-streaming PASSED")


@pytest.mark.asyncio
async def test_always_streaming_continuous_usage(
    always_client: openai.AsyncOpenAI, winning_call
):
    """Always policy: Streaming should return usage in EVERY chunk."""
    logger.info("TEST: Always policy streaming (continuous usage)")

    stream = await always_client.audio.transcriptions.create(
        model=MODEL_NAME,
        file=winning_call,
        language="en",
        temperature=0.0,
    ...
    async for chunk in stream:
        ...
        if not chunk.choices:
            final_chunk_count += 1
            assert hasattr(chunk, "usage") and chunk.usage is not None, (
                "Final chunk must have usage"
            )
        else:
            assert hasattr(chunk, "usage") and chunk.usage is not None, (
